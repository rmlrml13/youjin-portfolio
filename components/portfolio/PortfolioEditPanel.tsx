...
export default function PortfolioEditPanel({ project, token, onSaved, onDeleted, onClose }: Props) {
  ...
  return (
    <>
      ...
      <div style={{ position:'fixed', top:0, right:0, bottom:0, width:'min(420px, 100vw)', zIndex:9200, background:'#fff', boxShadow:'-8px 0 40px rgba(0,0,0,0.12)', display:'flex', flexDirection:'column', animation:'slideIn 0.25s ease' }}>
        ...
        <div style={{ padding:'1.5rem 2rem', borderBottom:'1px solid #E0DED8', display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
          <div>
            <p style={{ fontSize:'10px', letterSpacing:'0.12em', textTransform:'uppercase', color:'#888880', margin:0 }}>{isNew ? '새 프로젝트' : '프로젝트 수정'}</p>
            <h3 style={{ fontFamily:'DM Serif Display, serif', fontSize:'1.1rem', margin:'0.2rem 0 0' }}>{isNew ? 'New Project' : form.title || '—'}</h3>
          </div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'1.5rem 2rem' }}>
          {/* 이미지 업로드 */}
          <div onClick={() => fileRef.current?.click()} style={{ width:'100%', aspectRatio:'4/3', background:'#ECEAE4', overflow:'hidden', marginBottom:'1rem', cursor:'pointer', position:'relative', display:'flex', alignItems:'center', justifyContent:'center' }}>
            {imagePreview
            ...
            <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap' }}>
              {COL_OPTIONS.map(o => (
                <button key={o.value} onClick={() => setForm(p => ({ ...p, col_size: o.value }))}
                  style={{ padding:'0.35rem 0.8rem', background: form.col_size === o.value ? '#1A1A18' : 'transparent', color: form.col_size === o.value ? '#F5F4F0' : '#888880', fontFamily:'DM Mono, monospace', fontSize:'10px', letterSpacing:'0.08em', textTransform:'uppercase', border:`1px solid ${form.col_size === o.value ? '#1A1A18' : '#E0DED8'}`, cursor:'pointer' }}>
                  {o.label}
                </button>
              ))}
            </div>
