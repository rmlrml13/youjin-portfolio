...
export default function AdminPage() {
  ...
  function resetForm() { setEditId(null); setForm(EMPTY_FORM) }
  ...
  async function deleteProject(id: number, title: string) {
    ...
    if (res.ok) { showToast('✓ 삭제 완료'); resetForm(); fetchProjects(token) }
  }
    ...
    <div style={s.loginWrap}>
      <div style={s.loginBox}>
        ...
        <input style={s.input} value={username} onChange={e => setUsername(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && doLogin()} placeholder="admin" />
      ...
      <div style={s.adminBody}>
        ...
        <div style={s.formPanel}>
          ...
          <div style={s.imgPreview}>
            {form.image_url
              ...
              : <span style={{ fontSize:'11px', color:'var(--muted)', letterSpacing:'0.08em', textTransform:'uppercase' }}>이미지 없음</span>
            }
          </div>
          ...
          <div style={{ marginBottom: '1rem' }}>
            ...
            <select style={s.input} value={form.col_size}
              onChange={e => setForm(prev => ({ ...prev, col_size: e.target.value }))}>
          ...
          <div style={s.formActions}>
            <button style={s.btnPrimary} onClick={saveProject}>저장</button>
        ...
        <div style={s.listPanel}>
          ...
          <div style={s.projectGrid}>
            {projects.length === 0 && (
              <div style={{ gridColumn:'1/-1', textAlign:'center', padding:'4rem', color:'var(--muted)', fontSize:'11px' }}>
                등록된 프로젝트가 없습니다
              </div>
            )}
            {projects.map((p, i) => (
              <div key={p.id} style={{ ...s.card, ...(editId === p.id ? s.cardSelected : {}) }}
...
const s: Record<string, React.CSSProperties> = {
  ...
  label:         { display:'block', fontSize:'10px', letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--muted)', marginBottom:'0.5rem' },
  input:         { width:'100%', padding:'0.75rem 1rem', border:'1px solid var(--border)', background:'var(--bg)', fontFamily:'DM Mono, monospace', fontSize:'13px', color:'var(--fg)', outline:'none', marginBottom:'1rem' },
  errMsg:        { color:'#C0392B', fontSize:'11px', marginBottom:'0.8rem' },
  btnPrimary:    { padding:'0.75rem 1.5rem', background:'var(--fg)', color:'var(--bg)', fontFamily:'DM Mono, monospace', fontSize:'11px', letterSpacing:'0.12em', textTransform:'uppercase', border:'none', cursor:'pointer', width:'100%' },
  btnSecondary:  { padding:'0.75rem 1.5rem', background:'transparent', color:'var(--fg)', fontFamily:'DM Mono, monospace', fontSize:'11px', letterSpacing:'0.12em', textTransform:'uppercase', border:'1px solid var(--border)', cursor:'pointer', flex:1 },
  btnSm:         { padding:'0.4rem 0.8rem', background:'transparent', color:'var(--fg)', fontFamily:'DM Mono, monospace', fontSize:'10px', letterSpacing:'0.1em', textTransform:'uppercase', border:'1px solid var(--border)', cursor:'pointer' },
  btnDanger:     { color:'#C0392B', borderColor:'#C0392B' },
  adminHeader:   { position:'sticky', top:0, zIndex:100, background:'var(--fg)', color:'var(--bg)', padding:'1rem 2rem', display:'flex', justifyContent:'space-between', alignItems:'center' },
  adminLogo:     { fontFamily:'DM Serif Display, serif', fontSize:'1.1rem', color:'var(--bg)' },
  adminHeaderLink: { color:'rgba(255,255,255,0.6)', textDecoration:'none', fontSize:'11px', letterSpacing:'0.1em', textTransform:'uppercase' },
  logoutBtn:     { background:'none', border:'1px solid rgba(255,255,255,0.3)', color:'rgba(255,255,255,0.8)', fontFamily:'DM Mono, monospace', fontSize:'10px', letterSpacing:'0.1em', textTransform:'uppercase', padding:'0.4rem 1rem', cursor:'pointer' },
  ...
  imgPreview:    { width:'100%', aspectRatio:'4/3', background:'var(--panel, #ECEAE4)', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:'1rem', overflow:'hidden' },
  ...
  cardThumb:     { aspectRatio:'4/3', background:'var(--panel, #ECEAE4)', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' },
  cardInfo:      { padding:'0.8rem 1rem', borderTop:'1px solid var(--border)' },
  cardTag:       { fontSize:'9px', letterSpacing:'0.12em', textTransform:'uppercase', color:'var(--muted)', marginBottom:'0.2rem' },
  cardTitle:     { fontFamily:'DM Serif Display, serif', fontSize:'0.95rem', marginBottom:'0.5rem' },
  toast:         { position:'fixed', bottom:'2rem', right:'2rem', background:'var(--fg)', color:'var(--bg)', padding:'0.8rem 1.5rem', fontSize:'11px', letterSpacing:'0.08em', zIndex:999 },
}
