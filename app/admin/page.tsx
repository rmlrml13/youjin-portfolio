...
export default function AdminPage() {
  ...
  async function saveProject() {
    if (!form.title || !form.tag || !form.year) {
      alert('제목, 태그, 연도는 필수입니다.')
      return
    }
    setSaving(true)
    ...
    fd.append('col_size',   form.col_size)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={s.adminBody}>
        ...
        <div style={s.formPanel}>
          ...
          {/* 이미지 업로드 영역 */}
          <div
            style={{ ...s.imgPreview, cursor: 'pointer', position: 'relative' }}
          ...
          >
            ...
            {imagePreview && (
              <div style={s.imgOverlay}>
                <span style={{ fontSize:'11px', color:'#fff', letterSpacing:'0.08em', textTransform:'uppercase' }}>
                  클릭하여 변경
                </span>
              </div>
            )}
          ...
          {imageFile && (
            <p style={{ fontSize:'10px', color:'var(--muted)', marginBottom:'1rem', letterSpacing:'0.06em' }}>
              📎 {imageFile.name}
            </p>
          )}
        ...
        <div style={s.listPanel}>
          ...
          <div style={s.projectGrid}>
            ...
            {projects.map((p, i) => (
              <div
                key={p.id}
                style={{ ...s.card, ...(editId === p.id ? s.cardSelected : {}) }}
                ...
                <div style={s.cardInfo}>
                  ...
                  <div style={{ display:'flex', gap:'0.4rem' }}>
                    <button style={s.btnSm}
                      onClick={e => { e.stopPropagation(); selectProject(p) }}>수정</button>
                    <button style={{ ...s.btnSm, ...s.btnDanger }}
                      onClick={e => { e.stopPropagation(); deleteProject(p.id, p.title) }}>삭제</button>
                  </div>
                </div>
              </div>
            ))}
...
const s: Record<string, React.CSSProperties> = {
  ...
  input:           { width:'100%', padding:'0.75rem 1rem', border:'1px solid var(--border)', background:'var(--bg)', fontFamily:'DM Mono, monospace', fontSize:'13px', color:'var(--fg)', outline:'none', marginBottom:'0' },
  errMsg:          { color:'#C0392B', fontSize:'11px', marginBottom:'0.8rem' },
  btnPrimary:      { padding:'0.75rem 1.5rem', background:'var(--fg)', color:'var(--bg)', fontFamily:'DM Mono, monospace', fontSize:'11px', letterSpacing:'0.12em', textTransform:'uppercase' as const, border:'none', cursor:'pointer', flex:1 },
  ...
  imgOverlay:      { position:'absolute' as const, inset:0, background:'rgba(0,0,0,0.4)', display:'flex', alignItems:'center', justifyContent:'center', opacity:0, transition:'opacity 0.2s' },
