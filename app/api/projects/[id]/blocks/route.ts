...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    const fileName = `projects/${params.id}/blocks/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`
    ...
    const { data, error } = await supabase
      .from('project_blocks')
      .insert([{ project_id: Number(params.id), type: 'image', content: '', image_url: urlData.publicUrl, sort_order }])
      .select().single()
